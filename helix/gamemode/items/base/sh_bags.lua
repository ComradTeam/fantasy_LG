		...
		if (index) then
			...
			if (inventory and inventory.slots) then
				...
				if (parent == ix.gui.openedStorage or item.name == "Suitcase") then
					...
					DFrame:SetSize(item.invWidth * (iconSize + 2) + SScaleMin(4 / 3), item.invHeight * (iconSize + SScaleMin(2 / 3)) + SScaleMin(30 / 3)) -- 4 and 30 are accounting for the size of DFrame borders here
...
ITEM.postHooks.drop = function(item, result)
	...
	if item.name != "Suitcase" then
		local index = item.player:FindBodygroupByName(item.bodygroup)
		...
		if (index > -1) then
			...
			item.player:SetBodygroup(index, 0)
...
function ITEM:OnTransferred(curInv, inventory)
	...
	if bagInventory:GetOwner() then
		if self.name != "Suitcase" then
			local owner = bagInventory:GetOwner()
			local index = owner:FindBodygroupByName(self.bodygroup)
