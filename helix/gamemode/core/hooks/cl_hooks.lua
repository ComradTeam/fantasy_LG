--[[
Server Name: Willard Networks - BETA
Server IP:   178.239.172.10:27015
File Path:   gamemodes/helix/gamemode/core/hooks/cl_hooks.lua
...
local LOWERED_ANGLES = Angle(-10, 10, -10)
	...
	timer.Create("ixCheckTargetEntity", 0.1, 0, function()
		...
		if (bShouldShow and IsValid(lastEntity) and hookRun("ShouldPopulateEntityInfo", lastEntity) != false and
			...
			if (!IsValid(panel) or (IsValid(panel) and panel:GetEntity() != lastEntity)) then
				...
				local infoPanel = vgui.Create(ix.option.Get("minimalTooltips", true) and "ixTooltipMinimal" or "ixTooltip")
...
function GM:HUDPaintBackground()
	...
	if (IsValid(weapon) and hook.Run("CanDrawAmmoHUD", weapon) != false and weapon.DrawAmmo != false) then
		local configpos = ix.option.Get("AmmoPosition", "Bottom Right")
		local paddingX, paddingY = SScaleMin(30 / 3), SScaleMin(30 / 3)

		local scale64 = SScaleMin(64 / 3)
		local scale32 = scale64 / 2
		local scale128 = scale64 * 2
		...
		local hudposX, hudposY = ScrW() - paddingX - ((secondary > 0) and scale64 or scale128), ScrH() - paddingY - scale64

		if configpos == "Top Right" then
			hudposX = ScrW() - paddingX - ((secondary > 0) and scale64 or scale128)
			hudposY = paddingY
		elseif configpos == "Top Left" then
			hudposX = paddingX
			hudposY = paddingY
		elseif configpos == "Bottom Left" then
			hudposX = paddingX
			hudposY = ScrH() - paddingY - scale64
		end

		local x, y = hudposX, hudposY

		if (secondary > 0) then
			ix.util.DrawBlurAt(x, y, scale64, scale64)

			surface.SetDrawColor(255, 255, 255, 5)
			surface.DrawRect(x, y, scale64, scale64)
			surface.SetDrawColor(255, 255, 255, 3)
			surface.DrawOutlinedRect(x, y, scale64, scale64)

			ix.util.DrawText(secondary, x + scale32, y + scale32, nil, 1, 1, "SkillsDesc")
		end

		if (weapon:GetClass() != "weapon_slam" and clip > 0 or count > 0) then
			ix.util.DrawBlurAt(x, y, scale128, scale64)

			surface.SetDrawColor(255, 255, 255, 5)
			surface.DrawRect(x, y, scale128, scale64)
			surface.SetDrawColor(255, 255, 255, 3)
			surface.DrawOutlinedRect(x, y, scale128, scale64)

			ix.util.DrawText((clip == -1 or clipMax == -1) and count or clip.."/"..count, x + scale64, y + scale32, nil, 1, 1, "SkillsDesc")
		end
	end
