	...
	hook.Add("InitializedConfig", "ixChatTypes", function()
		...
		ix.chat.Register("ic", {
			...
			OnChatAdd = function(self, speaker, text)
				...
				local icon = "willardnetworks/chat/whisper_icon.png"
				...
				chat.AddText(icon, ix.config.Get("chatColor"), speaker:Name().." говорит ", color_white, ": "..text)
			end,
		...
		ix.chat.Register("me", {
			...
			OnChatAdd = function(self, speaker, text)
					local icon = ix.util.GetMaterial("willardnetworks/chat/pm_icon.png")

						chat.AddText(icon, Color(174, 156, 0), "** ".. "("..speaker:GetName()..") "..text)
			end,
			CanHear = ix.config.Get("chatRange", 280) * 2,
			prefix = {"/Me", "/Action"},
			description = "@cmdMe",
	...
	description = "@cmdEvent"
		...
		ix.chat.Register("ooc", {
			...
			OnChatAdd = function(self, speaker, text)
				...
				chat.AddText(icon, Color(255, 0, 0), "[НонРП/OOC] ", speaker, color_white, ": "..text)
			end,
			prefix = {"//", "/OOC"},
			description = "@cmdOOC",
			noSpaceAfter = true
...
-- RP.
			...
			OnChatAdd = function(self, speaker, text)
				...
				chat.AddText(icon, Color(0, 128, 0), "[RP] ", speaker, color_green, ": "..text)
			end,
			prefix = {"/rp"},
			description = "Используется для отыгровки глобальных РП действий.",
			noSpaceAfter = true
		})
		...
		ix.chat.Register("looc", {
			...
			OnChatAdd = function(self, speaker, text)
				local icon = ix.util.GetMaterial("willardnetworks/chat/message_icon.png")
...
ix.chat.Register("connect", {
	...
	OnChatAdd = function(self, speaker, text)
		...
		chat.AddText(icon, Color(128, 128, 128), L("playerConnected", text))
	end,
	noSpaceAfter = true
})
