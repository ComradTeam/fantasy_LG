...
if (SERVER) then
			...
			query:Callback(function(result, status, lastID)
				local invQuery = mysql:Insert("ix_inventories")
					...
					invQuery:Callback(function(invResult, invStats, invLastID)
						...
						ix.char.RestoreTableVars(data, data, lastID, true)
	...
	function ix.char.Restore(client, callback, bNoCache, id)
		...
		local query = mysql:Select("ix_characters")
			query:Select("id")
			ix.char.UpdateQueryWithVars(query)
			...
			query:Callback(function(result)
				...
				local charsToLoad = result and #result or 0
				for _, v in ipairs(result or {}) do
					...
					if (charID) then
						...
						local tblVarsQuery = mysql:Select("ix_characters_data")
						tblVarsQuery:Select("key")
						tblVarsQuery:Select("data")
						tblVarsQuery:Where("id", charID)
						tblVarsQuery:Callback(function(tblResult)
							ix.char.RestoreTableVars(data, tblResult or {}, charID)
							...
							charsToLoad = charsToLoad - 1
							if (charsToLoad == 0 and callback) then
								callback(characters)
							end
						end)
						tblVarsQuery:Execute()
					else
						charsToLoad = charsToLoad - 1
	...
	function ix.char.UpdateQueryWithVars(query)
		-- populate query
		for _, v in pairs(ix.char.vars) do
			if (v.field and v.fieldType and !v.bSaveLoadInitialOnly and type(v.default) != "table") then
				...
				if (v.FilterValues) then
					query:WhereIn(v.field, v:FilterValues())
				end
			end
		end
	end
	...
	function ix.char.RestoreVars(data, characterInfo)
		...
		for k, v in pairs(ix.char.vars) do
			if (v.field and characterInfo[v.field] and !v.bSaveLoadInitialOnly and type(v.default) != "table") then
	...
	function ix.char.RestoreTableVars(data, tblResult, id, bForceInsert)
		local characterInfo = tblResult
		for _, v in ipairs(tblResult) do
			characterInfo[v.key] = v.data
		end

		-- populate character data
		for k, v in pairs(ix.char.vars) do
			if (v.field and !v.bSaveLoadInitialOnly and type(v.default) == "table") then
				if (!characterInfo[v.field] or bForceInsert) then
					local tblQuery = mysql:Insert("ix_characters_data")
					tblQuery:Insert("data", characterInfo[v.field] and util.TableToJSON(characterInfo[v.field]) or "NULL")
					tblQuery:Insert("id", id)
					tblQuery:Insert("key", v.field)
					tblQuery:Execute()
				end

				if (characterInfo[v.field]) then
					local value = characterInfo[v.field]
					data[k] = istable(value) and value or util.JSONToTable(value)
				end
			end
		end
	end
	...
	if (SERVER) then
		...
		net.Receive("ixCharacterDelete", function(length, client)
			...
			if (character and character.steamID == steamID) then
				...
				query = mysql:Delete("ix_characters_data")
					query:Where("id", id)
				query:Execute()
